...
#ifdef ENABLE_TASKS
/* 
 * Based on SerialIntegratedPipeline.
 */

void* 

void duplicateAndCompress(void* _chunk) {


}

void *FragmentRefine(void* _chunk) {
  int r=0;
  chunk_t *temp;
  u32int * rabintab = malloc(256*sizeof rabintab[0]);
  u32int * rabinwintab = malloc(256*sizeof rabintab[0]);
  ...
  //get one item from argument.
  chunk_t* chunk = (chunk_t*)_chunk;
  assert(chunk!=NULL);
...
void *TaskParallelEncode(void * targs) {
  struct thread_args *args = (struct thread_args *)targs;
  size_t preloading_buffer_seek = 0;
  int fd = args->fd;
  int fd_out = create_output_file(conf->outfile);
