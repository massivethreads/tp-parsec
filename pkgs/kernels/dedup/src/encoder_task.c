...
#define TASK_CUTOFF_FRAGMENT 10
#define TASK_CUTOFF_FRAGMENT_REFINE 10
 ...
 */
void DeduplicateAndCompress(chunk_t * chunk_in, int chunk_num) {
  cilk_begin;
  chunk_t *chunk = chunk_in;
#ifdef ENABLE_STATISTICS
 ...
 */
void FragmentRefine(chunk_t * chunk_in, int chunk_num) {
  cilk_begin;
  int r;
  ...
  while (1) {
    ...
    while (1) {
      ...
      if(conf->preloading) {
        ...
        memcpy(chunk->uncompressed_data.ptr+bytes_left, input_file_buffer+preloading_buffer_seek, max_read);
        bytes_read = max_read;
...
SYNCHRONIZE:
    ...
    //Now chunks is processed.
    call_task0(spawn FragmentRefine(child_task_chunk_head, child_task_chunk_num));
    wait_tasks;
