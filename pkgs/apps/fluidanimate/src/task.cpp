...
//Modified by Shintaro Iwasaki, from Original pthreads.cpp
...
//Uncomment to add code to check that Courant窶擢riedrichs窶鏑ewy condition is satisfied at runtime
...
unsigned int GetPow2(unsigned int tasknum)
{
    ...
    for(int iy = grids[tid].sy; iy < grids[tid].ey; ++iy)
      for(int ix = grids[tid].sx; ix < grids[tid].ex; ++ix)
      {
        ...
        for(int j = 0; j < np2; ++j)
        {
          ...
          {
            std::cerr << "FATAL ERROR: Courant窶擢riedrichs窶鏑ewy condition not satisfied." << std::endl;
            exit(1);
          }
...
void AdvanceFramesMT(int tasknum, int framenum)
{
